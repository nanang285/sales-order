            ...
            <div class="container mx-auto px-6 lg:px-8 relative">
                <div class="flex flex-col md:flex-row lg:mb-16 items-center">
                    <div data-aos="fade-up" data-aos-anchor-placement="top-center" class="md:w-1/2 p-6 space-y-6">
                        <h2 class="text-primary text-3xl font-bold mb-4 underline decoration-wavy decoration-blue-900">
                            Tentang Kami</h2>
                        ...
                        @if ($aboutUs->list_items ?? '')
                            <ul class="list-disc list-outside pl-6 text-lg font-semibold mt-4 space-y-2 text-gray-700"
                ...
                <div data-aos="fade-up" data-aos-anchor-placement="top-center"
                    ...
                    <div class="md:w-1/2 p-6 md:p-8 mt-8 md:mt-0">
                        <h2
                            class="text-2xl font-extrabold mb-4 text-primary border-b-4 border-blue-900 border-opacity-70 inline-block">
                            Visi & Misi Kami
                        </h2>
                        <p class="mb-6 text-lg md:text-xl font-light text-blue-900 leading-relaxed"
                            style="text-align: justify;">
                            {{ $Vission->description ?? '' }}
                        </p>
                        @if ($Vission->list_items ?? '')
                            <ul class="list-outside list-disc pl-6 space-y-4">
                                @foreach (json_decode($Vission->list_items) as $item)
                                    <li class="text-base md:text-lg font-semibold text-gray-700"
                                        style="text-align: justify;">
                                        {{ $item }}
                                    </li>
                                @endforeach
        ...
        <div class="py-8 relative overflow-hidden">
            <div class="container mx-auto px-6 lg:px-8 relative">
                <div class="relative mx-auto max-w-full lg:text-center mb-12">
                    <p
                        class="absolute no-select inset-0 text-6xl lg:text-8xl font-extrabold text-gray-300 opacity-30 tracking-tighter text-center z-0 transform -translate-y-6 lg:-translate-y-7">
                        OUR TEAM
                    </p>
                    <p
                        class="relative no-select text-3xl lg:text-4xl font-extrabold tracking-tight text-gray-900 text-center z-10">
                    ...
                    @foreach ($ourTeam as $team)
                        <div data-aos="fade-up" data-aos-anchor-placement="top-center"
                            ...
                            <img class="object-cover shadow w-full h-auto transform transition-transform duration-300 ease-in-out group-hover:scale-105 lg:group-hover:scale-110"
                                src="{{ asset('storage/uploads/our-team/' . ($team->image_path ?? '')) }}"
                                alt="zmi-team-images">
                            <div
                                class="absolute inset-x-0 py-4 bottom-5 flex flex-col items-center justify-center bg-gradient-to-t bg-gray-800 bg-opacity-80 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
