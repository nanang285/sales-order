        ...
        <div class="relative bg-white rounded-lg shadow p-5">
            ...
            <div class="">
                <form class="space-y-4" action="{{ route('admin.homepages.galery.store') }}" method="POST"
                    ...
                    <div>
                        <label for="subtitle_{{ $galery->id ?? '' }}"
                            class="block mb-2 text-sm font-medium text-gray-900">Subtitle</label>
                        <textarea name="subtitle" id="subtitle_{{ $galery->id ?? '' }}"
                            ...
                            placeholder="Masukan subtitle" required maxlength="250">{{ $galery->subtitle ?? '' }}</textarea>
                    </div>
                    <div>
                        <label for="images"
        ...
        <div class="relative p-4 w-full max-w-lg max-h-full">
            <div class="relative bg-white rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button"
                        class="end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                        data-modal-hide="edit_modal_{{ $galery->id }}">
                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 14 14">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                    ...
                    <form class="space-y-4" action="{{ route('admin.homepages.galery.update', $galery->id) }}" method="POST"
                        ...
                        <div>
                            <label for="title_{{ $galery->id }}"
                                class="block mb-2 text-sm font-medium text-gray-900">Title</label>
                            <input type="text" name="title" id="title_{{ $galery->id }}"
                                value="{{ $galery->title }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400"
                                required />
                        </div>
                        <div>
                            ...
                            <textarea name="subtitle" id="subtitle_{{ $galery->id }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full h-24 p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400"
                                required maxlength="250">{{ $galery->subtitle }}</textarea>
                        </div>
                        <div>
                            <label for="image_{{ $galery->id }}" class="block mb-2 text-sm font-medium text-gray-900">Images</label>
                            <div class="flex items-center justify-center w-full">
                                <label for="image_{{ $galery->id }}"
                                    class="flex flex-col items-center justify-center w-full h-24 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                        <span class="w-8 h-8">
                                            <i class="fa-solid fa-cloud-arrow-up"></i>
                                    ...
                                    <input id="image_{{ $galery->id }}" type="file" name="image_path" class="hidden" accept="image/*" />
                                </label>
                            </div>
                        <label class="block mt-2 text-center text-sm font-medium text-red-600">Gambar (MAX 4MB)</label>
                        </div>
...
{{-- Hapus Data --}}
@foreach ($galerySection as $galery)
    <div id="delete_modal_{{ $galery->id }}" tabindex="2" aria-hidden="true"
        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative w-full max-w-lg max-h-full">
            ...
            <div class="relative bg-white p-4 rounded-lg shadow">
                ...
                <div class="flex items-center justify-between mb-2">
                    ...
                    <button type="button"
                        class="end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                        data-modal-hide="delete_modal_{{ $galery->id }}">
                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 14 14">
                ...
                <div class="">
                    ...
                    <form class="mt-4" action="{{ route('admin.homepages.galery.destroy', $galery->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="subtitle" id="subtitle_{{ $galery->id }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full h-24 p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400"
                                readonly>{{ $galery->subtitle }}</textarea>
                        </div>
                        <div class="mb-3">
                            <label for=""
                                class="block mb-2 text-sm font-medium text-gray-900">Images</label>
